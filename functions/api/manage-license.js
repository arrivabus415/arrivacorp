...
export async function onRequestPost({ request }) {
  ...
  const { username, product, granted } = await request.json();
  if (!username || !product) {
  ...
  await fetch(`${FIREBASE_DB_URL}/${id}/products/${product}.json`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      whitelisted: granted,
      grantedBy: granted ? admin : null,
      revokedBy: granted ? null : admin,
